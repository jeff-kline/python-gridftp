...
environ.update(env)
dir_host = "gsiftp://%s:%d" % (getfqdn(), gridftp_server.port)
pfn_list = [''.join([dir_host, '/tmp/dir_foo/lfn.gwf']),
            ''.join([dir_host, '/tmp/dir_foo/baz/lfn.gwf']),
            ''.join([dir_host, '/tmp/dir_foo/bar/1/2/3/4/5/6/6/7/8/lfn.gwf']),
            ''.join([dir_host, '/tmp/dir_foo/bar/1/2/3/4/5/3/6/7/8/lfn.gwf']),
            ''.join([dir_host, '/tmp/dir_foo/bar/1/2/3/4/5/2/6/7/8/lfn.gwf']),
            ''.join([dir_host, '/tmp/dir_foo/bar/1/2/3/4/5/2/0/7/8/lfn.gwf']),
            ]
try:
    op = OperationAttr()
    ...
    mkdir_event = Event()
    mkdir_err = bytearray()
    def mkdir_cb(arg, handle, error):
        if error is not None:
            if 'a system call failed: file exists' in error.lower():
                pass
            else:
                mkdir_err[:] = str(error)
        mkdir_event.set()

    dst = dirname(pfn_list[0])
    # idempotence
    for j in range(5):
        cli.mkdir(dst, mkdir_cb, None, op)
        mkdir_event.wait()
        mkdir_event.clear()
        if mkdir_err:
            raise RuntimeError(mkdir_err)
    try: rmdir(dst)
    except OSError: pass
    ...
    _dirs_to_make = set()
    for pfn in pfn_list:
        scheme, netloc, path, params, query, fragment = urlparse(pfn)
        path_l = dirname(path).split('/')
        d = []
        for p in path_l:
            d.append(p)
            newdir = urlunparse((scheme, netloc, '/'.join(d), params, query, fragment))
            _dirs_to_make.add(newdir)
    dirs_to_make = sorted(_dirs_to_make)
    
    for d in dirs_to_make:
        print d
        cli.mkdir(d, mkdir_cb, None, op)
        mkdir_event.wait()
        mkdir_event.clear()
        if mkdir_err:
            raise RuntimeError(mkdir_err)
    #for d in reversed(dirs_to_make):
    #    try: rmdir(d[len(dir_host):])
    #    except OSError: pass
